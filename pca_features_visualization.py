...
parser.add_argument("--image_path", type=str, default='./data/aligned/source14.png')
...
save_path = os.path.join(args.save_path, f'{sformat(args.model_name)}_feature_norm01_'+os.path.basename(args.image_path))
...
def visualize(args, m, layer):
    ...
    # obtain embeddings
    embeddings = {
        'k': vit_extractor.get_keys_from_input,
        'q': vit_extractor.get_queries_from_input,
        'v': vit_extractor.get_values_from_input,
        'f': vit_extractor.get_features_from_input
    }

    with torch.no_grad():
        try:
            input_img_ = preprocess(input_img)
            embedding = embeddings[m](input_img_, layer)
        except:
            input_img_ = preprocess(input_img).half()
            embedding = embeddings[m](input_img_, layer)
        h, t, d = embedding.shape
        concatenated = embedding.transpose(0, 1).reshape(t, h * d)
        concatenated /= concatenated.norm(dim=[0, 1], keepdim=True)

    pca = PCA(n_components=3)
    pca.fit(concatenated.cpu().numpy())
    components = pca.transform(concatenated.cpu().numpy())
